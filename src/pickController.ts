import { createLtlAnalyzer, LtlAnalyzer } from './ltlAnalyzer';
...
export class PickController {
  ...
  constructor() {
    this.analyzer = createLtlAnalyzer();
  ...
  async generateCandidates(
    ...
    // Ensure the LTL engine module is loaded so synchronous verifyMatch works.
      ...
      for (let j = i + 1; j < candidatePatterns.length && !foundMinThreshold; j++) {
        try {
          ...
          const countPromise = (async () => {
            ...
            // Do NOT early-stop when pairThreshold === 1: evaluate every pair so each
            // candidate gets its true minimum threshold (otherwise candidates not yet
            // reached keep the default threshold, giving inconsistent elimination
            // sensitivity). The timeout branch below still bails via foundMinThreshold.
          })();
